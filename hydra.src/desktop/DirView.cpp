...
DirView::DirView(FileList *_filelist)
  : View(_filelist)
{
  initGui();
...
{
  QString existingBase( dm_model.filePath(dm_tree->selectionModel()->currentIndex()) );

  if (fl->baseDir() == existingBase)
    return;

  dm_tree->selectionModel()->setCurrentIndex(dm_model.index(fileList()->baseDir()), QItemSelectionModel::SelectCurrent);
}
...
void DirView::onMenuOpenDir(void)
{
  ...
  QString newdir( dm_model.filePath(dm_tree->selectionModel()->currentIndex()) );
...
void DirView::onRefreshDir(void)
{
...
{
  bool ok = false;
  QString newname = QInputDialog::getText(this, "Create Folder", "Folder name:", QLineEdit::Normal, "Untitled", &ok);
...
void DirView::onRenameDir(void)
{
  ...
  bool ok = false;
  QString newname = QInputDialog::getText(this, "Rename Folder", "Folder name:", QLineEdit::Normal, curname, &ok);
...
void DirView::onDelDir(void)
{
  ...
  bool success = FileOpDisk().delDir(dm_model.filePath(dm_tree->currentIndex()));

  if (success)
    dm_model.refresh(dm_tree->currentIndex().parent());
  else
    QMessageBox::critical(this, "Directory Delete Error",
        "There was an error in deleting the directory. Note, only empty "
        "directories can be deleted.");
}

void DirView::onCopySelection(void)
{
  doFileOp(FileOp::Op_Copy);
}

void DirView::onMoveSelection(void)
{
  doFileOp(FileOp::Op_Move);
}
...
void DirView::initGui(void)
{
  ...
  connect(dm_tree, SIGNAL(activated(const QModelIndex&)), this, SLOT(onActivated(const QModelIndex&)));
