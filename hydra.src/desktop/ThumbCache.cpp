...
ThreadedThumbCache::ThreadedThumbCache(void)
  : dm_cache(1000)
{
}
...
desktop::cache_ptr<QPixmap> ThreadedThumbCache::getPixmap(const QString &fullfilename, const QString &hash, int rotateCode, int windoww, int windowh, FileList *fileList, int indexOfFile)
{
  ...
  RunnableEventFunction::enqueueWorker(
      std::bind(std::mem_fn(&ThreadedThumbCache::workerFunc), this, fullfilename, thumbName, hash, rotateCode, windoww, windowh, fileList, indexOfFile),
      RunnableEvent::nextPriority());

  return desktop::cache_ptr<QPixmap>();
}

void ThreadedThumbCache::workerFunc(QString fullfilename, QString thumbName, QString hash, int rotateCode, int windoww, int windowh, FileList *fileList, int indexOfFile)
{
  {
    QMutexLocker L(&dm_pendingjobs_lock);
  ...
  RunnableEventFunction::enqueueMain(
      std::bind(std::mem_fn(&ThreadedThumbCache::mainFunc),
        this, fullfilename, thumbName, img, fileList, indexOfFile));
}

void ThreadedThumbCache::mainFunc(QString fullfilename, QString thumbName, QImage img, FileList *fileList, int indexOfFile)
{
