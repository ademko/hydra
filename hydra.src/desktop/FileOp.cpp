...
{
  switch (op) {
    case Op_Del: return delFile(srcfile);
    case Op_Move: return moveFile(srcfile, destfile);
    case Op_Copy: return copyFile(srcfile, destfile);
  }
  assert(false);
  return false;
}
...
FileOpGuiTest::FileOpGuiTest(QWidget *parent)
  : dm_parent(parent)
{
...
bool FileOpDB::copyFile(const QString &srcfile, const QString &destfile)
{
  ...
  if (Engine::instance()->getFileItem(srcfile, 0, 0, &pathrec) != Engine::Load_OK)
    return false;
...
int desktop::guiFileOpFiles(QWidget *parent, int op, const QStringList &srcfiles, 
      const QString &destDir, FileOp *handler)
{
  int i = 0, done = 0;
  QString dest_name;
  ...
  try {
    for (QStringList::const_iterator ii=srcfiles.begin(); ii != srcfiles.end(); ++ii) {
      i++;
      ...
      if (!tester.opFile(op, *ii, dest_name) || !disker.opFile(op, *ii, dest_name))
        continue;
