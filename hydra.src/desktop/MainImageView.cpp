...
MainImageView::MainImageView(MainWindow *mainwin, FileList *_filelist,
                             ImageCache *_loader)
    ...
    connect(_filelist->selectionModel(),
...
void MainImageView::resizeEvent(QResizeEvent *event) { dm_dirty = true; }
...
void MainImageView::paintEvent(QPaintEvent *event) {
    ...
    if (dm_dirty) {
        ...
        if (fileList()->isValid()) {
            dm_image =
                dm_loader->getImage(fileList()->selectedFile().fullfilename());

            if (dm_image->isNull())
                dm_pixmap = QPixmap();
            else {
                // rotate as needed
                QImage rotimage = hydra::rotateImageByCode(
                    fileList()->selectedFile().rotateCode(), *dm_image);
                dm_pixmap = ImageCache::getPixmap(rotimage, dc.window().width(),
    ...
    if (dm_pixmap.isNull()) {
        ...
        dc.drawPixmap((dc.window().width() - dm_pixmap.width()) / 2,
                      (dc.window().height() - dm_pixmap.height()) / 2,
                      dm_pixmap.width(), dm_pixmap.height(), dm_pixmap);
}
...
void MainImageView::keyPressEvent(QKeyEvent *event) {
    if (isFullScreen() && event->modifiers() == Qt::CTRL &&
        event->key() == Qt::Key_F) {
        // bug fix:
        // ctrl-f accelrator does not work when MainImageView is full screen and
        // thus its own top level window. need to do ctrl f handling directly
        mainWindow()->toggleFullScreen();
    }
