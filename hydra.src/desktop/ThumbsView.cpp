...
ThumbsDelegate::ThumbsDelegate(FileList *_filelist,
                               ThreadedThumbCache *_thumbcache)
    : dm_filelist(_filelist), dm_thumbcache(_thumbcache) {}

void ThumbsDelegate::paint(QPainter *painter,
                           const QStyleOptionViewItem &option,
                           const QModelIndex &index) const {
    // QString fullfilename(index.model()->data(index,
    // Qt::DisplayRole).toString());
    desktop::cache_ptr<QPixmap> pix;
    FileEntry &entry(dm_filelist->fileAt(index.row()));

    // qDebug() << option.rect << index.model()->data(index,
    // Qt::DisplayRole).toString();

    // draw title and selection rectangle
    painter->eraseRect(option.rect);

    painter->drawText(option.rect.x(), option.rect.y() + OFFSET_H * 2 + PIC_H,
                      option.rect.width(), OFFSET_H * 2,
                      Qt::AlignTop | Qt::AlignHCenter, entry.justname());

    if (option.state & QStyle::State_Selected) {
        painter->setPen(QPen(QBrush(Qt::black), 4));
        painter->drawRect(option.rect.x() + 2, option.rect.y() + 2, FULL_W - 4,
                          FULL_H - 4);
    }
    ...
    if (needsBigRead) {
        ...
        for (hydra::FileItemRecord::tags_t::const_iterator ii =
    ...
    if (!tags.isEmpty())
        painter->drawText(option.rect.x(),
                          ...
                          option.rect.width(), OFFSET_H * 2,
                          Qt::AlignTop | Qt::AlignHCenter, tags);
    ...
    if (dm_images.count(entry.recordHash()) == 0) {
        ...
        pix = dm_thumbcache->getPixmap(
            entry.fullfilename(), entry.recordHash(), entry.rotateCode(),
    ...
    // painter->drawText(option.rect.x()+0, option.rect.y()+10,
    // index.model()->data(index, Qt::DisplayRole).toString()); if ((option.state
    // & QStyle::State_Selected) != 0) painter->drawRect(option.rect.x(),
    // option.rect.y(), option.rect.width()-1, option.rect.height()-1);

    if (pix.get())
        painter->drawPixmap(
            option.rect.x() + OFFSET_W / 2 + (PIC_W - pix->width() / 2) / 2,
            option.rect.y() + OFFSET_H / 2 + (PIC_H - pix->height() / 2) / 2,
            pix->width() / 2, pix->height() / 2, *pix);
}

QSize ThumbsDelegate::sizeHint(const QStyleOptionViewItem &option,
                               const QModelIndex &index) const {
    // qDebug() << __FUNCTION__;
    return QSize(FULL_W, FULL_H);
}

void ThumbsDelegate::clear(void) { dm_images.clear(); }

void ThumbsDelegate::onImageChange(FileList *fl, int fileIndex) {
    imagecache_t::iterator ii =
        dm_images.find(fl->fileAt(fileIndex).recordHash());
    if (ii != dm_images.end())
        dm_images.erase(ii);
}
...
ThumbsView::ThumbsView(MainWindow *mainwin, FileList *_filelist,
                       ThreadedThumbCache *_thumbcache, bool wrap_list,
    ...
    connect(dm_list, SIGNAL(activated(const QModelIndex &)), this,
...
void ThumbsView::onActivated(const QModelIndex &i) {
    ...
    MainWindow *newmain =
        new MainWindow(MainWindow::VCR_VIEW, mainWindow()->mainStatePtr());
