...
FileEntry::FileEntry(const QString &_fullfilename)
    : dm_recordloaded(false), dm_fullfilename(_fullfilename),
      dm_justname(justName(_fullfilename)) {
    dm_fileSize = -1;
    dm_rotateCode = -1;
}
...
void FileEntry::loadRecord(bool *needsBigRead) const {
    ...
    // qDebug() << __FUNCTION__ << dm_fullfilename << (needsBigRead!=0) <<
    // dm_recordloaded;
    if (dm_recordloaded)
        return;

    // dm_failedrecord = !FileItemCache::instance()->getItem(dm_fullfilename,
    // dm_item, dm_hash, needsBigRead);
    ...
    if (dm_hash.isEmpty()) {
        ...
        // qDebug() << "FileEntry" << __FUNCTION__ << "getFileItem" <<
        // dm_fullfilename;
        code = Engine::instance()->getFileItem(dm_fullfilename, &dm_item, 0,
                                               /*&hashrec,*/ &pathrec);
        if (code == Engine::Load_OK)
            dm_hash = pathrec.hash;
    } else {
        // read the item record, by hash
        // qDebug() << "FileEntry" << __FUNCTION__ << "getFileItemByHash" <<
        // dm_hash;
    ...
    if (code != Engine::Load_OK) {
        ...
        // see if we need to add the item, which is considered a bigread as this
        // might trigger a checksum of the file contents
        // qDebug() << "FileEntry" << __FUNCTION__ << "addFile" <<
        // dm_fullfilename;
    ...
    // make it a 'successfull' read... this way, even if we get an error, we
    // wont keep trying to reread the records
