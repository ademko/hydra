...
TagSelectView::TagSelectView(FileList *_filelist) : View(_filelist) {
    initGui();
...
void TagSelectView::queryToSelection(FileList *fileList, hydra::Token *query,
    ...
    if (expandingselection)
        outselection.push_back(
            QItemSelectionRange(fileList->index(selectstart, 0),
                                fileList->index(fileList->numFiles() - 1,
