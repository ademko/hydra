...
SearchView::SearchView(FileList *_filelist)
    : View(_filelist),
      dm_last_query("not") // init dm_last_query to something invalid so that it
                           // gets reset right away
{
    initGui();
...
void SearchView::onClear(void) { dm_query->clear(); }

void SearchView::onFirst(void) { search(0, 1); }
...
void SearchView::onLast(void) { search(fileList()->numFiles() - 1, -1); }
...
void SearchView::search(int starti, int delta) {
    ...
    else if (starti >= fileList()->numFiles())
        starti = 0;
    ...
    while (true) {
        ...
        else if (curi >= fileList()->numFiles())
            curi = 0;
...
void SearchSelectView::onHighlight(void) {
    ...
    TagSelectView::queryToSelection(fileList(), dm_last_tok.get(),
                                    itemselection);

    fileList()->selectionModel()->select(itemselection,
