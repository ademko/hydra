...
FutureLoader::FutureLoader(ThreadedThumbCache *thumbcache, FileList *flist)
    : FileListListener(flist), dm_thumbcache(thumbcache), dm_flist(flist) {
...
void FutureLoader::start(void) {
    ...
    RunnableEventFunction::enqueueWorker(
        std::bind(std::mem_fn(&FutureLoader::workerFunc), this), -1);
}
...
void FutureLoader::mainFunc(void) {
    ...
    if (dm_hashstage) {
        ...
        int totest = 10; // quick check at most 100 files per pass (we are in
                         // the main gui thread after all)
        bool needsBigRead;
        ...
        while (dm_lastdoneindex < numFiles && totest > 0) {
            ...
            // qDebug() << "FutureLoader.record" << entry.fullfilename() << "@"
            // << dm_lastdoneindex;
            entry.containsRecord(&needsBigRead);
            if (needsBigRead) {
                totest = 0;
                // qDebug() << "FutureLoader" << "enqueue_background_hash" <<
                // entry.fullfilename();
    ...
    if (!dm_hashstage) {
        int totest = 100; // quick check at most 100 files per pass (we are in
                          // the main gui thread after all)
        ...
        while (dm_lastdoneindex < numFiles && totest > 0) {
            ...
            // qDebug() << "FutureLoader.thumb" << entry.fullfilename() << "@"
            // << dm_lastdoneindex;
            // check thumb info
            if (!dm_thumbcache->containsPixmap(
                    /*entry.fullfilename(),*/ entry.recordHash(),
                    entry.rotateCode(), hydra::Thumb::DEFAULT_THUMB_W,
                    hydra::Thumb::DEFAULT_THUMB_H)) {
                totest = 0;

                // qDebug() << "FutureLoader" << "enqueue_backgroud_thumb" <<
                // entry.fullfilename();
                dm_thumbcache->getPixmap(
                    entry.fullfilename(), entry.recordHash(),
                    entry.rotateCode(), hydra::Thumb::DEFAULT_THUMB_W,
                    hydra::Thumb::DEFAULT_THUMB_H, dm_flist, dm_lastdoneindex);
            } else {
                --totest;
            }
