...
MainWindow::MainWindow(short initMode, std::shared_ptr<MainState> state)
    : dm_state(state), dm_leftlist(dm_state->dm_leftlist), dm_fullscreenview(0),
      dm_slideshow_active(false) {
    ...
    // connect(dm_leftlist.selectionModel(), SIGNAL(currentChanged(const
    // QModelIndex&,const QModelIndex&)), this, SLOT(onCurrentChanged()));
    // needed to keep the counters up to date too
    connect(dm_leftlist.selectionModel(),
...
void MainWindow::toggleFullScreen(void) {
    if (dm_fullscreenview) {
        ...
        dm_fullscreenview =
            new MainImageView(this, &dm_leftlist, &dm_state->dm_imagecache);
...
void MainWindow::onAbout(void) {
    ...
    lay->addWidget(lab);
    lab = new QLabel("Copyright " + COPYRIGHT +
                     " " HYDRA_COPYRIGHT_STRING ", Aleksander B. Demko");
    lay->addWidget(lab);
...
void MainWindow::onQuit(void) { QApplication::quit(); }

void MainWindow::onMenuOpenDir(void) {
    QString newdir(QFileDialog::getExistingDirectory(this, "Open a folder",
                                                     dm_leftlist.baseDir()));
...
void MainWindow::onMenuImage(void) {
    ...
    case 2:
        ...
        break;
    case 3:
        dm_leftlist.setSelectedFile(static_cast<int>(dm_leftlist.numFiles()) -
                                    1);
        break;
    }
}
...
void MainWindow::onMenuRotate(void) {
    ...
    if (id == 0) {
        ...
        did_something = item.tags.insertKey(
            "rotate", QString::number(rotateCodeToDegrees(newCode)),
            &imageChanged);
    }
...
void MainWindow::initActions(void) {
    ...
    connect(dm_file_directory, SIGNAL(triggered(bool)), this,
            SLOT(onMenuOpenDir()));

    dm_file_directory_r =
        new QAction(QApplication::style()->standardIcon(QStyle::SP_DirOpenIcon),
                    "Open Folder (and &sub-folders)", this);
    dm_file_directory_r->setStatusTip(
        "Open a folder from disk (and all its sub-folders)");
    dm_file_directory_r->setProperty("id", 1);
    connect(dm_file_directory_r, SIGNAL(triggered(bool)), this,
            SLOT(onMenuOpenDir()));

    dm_file_directory_popout =
        new QAction(QApplication::style()->standardIcon(QStyle::SP_DirOpenIcon),
                    "Open Folder Browser Window", this);
    dm_file_directory_popout->setShortcut(QKeySequence(Qt::CTRL + Qt::Key_D));
    connect(dm_file_directory_popout, SIGNAL(triggered(bool)), this,
            SLOT(onMenuOpenDirBrowser()));
    ...
    connect(dm_fullscreen_action, SIGNAL(triggered(bool)), this,
    ...
    connect(dm_slideshow_action, SIGNAL(triggered(bool)), this,
            SLOT(toggleSlideShow()));
    ...
    connect(dm_rotate_reset, SIGNAL(triggered(bool)), this,
            SLOT(onMenuRotate()));
    ...
    connect(dm_rotate_left, SIGNAL(triggered(bool)), this,
            SLOT(onMenuRotate()));
    ...
    connect(dm_rotate_right, SIGNAL(triggered(bool)), this,
            SLOT(onMenuRotate()));

    dm_image_first = new QAction(
    ...
    for (int x = 0; x < dm_allplugins.size(); ++x) {
        ...
        case MainPlugin::menuSelection:
            dm_selectionmenu.push_back(action);
            break;
        case MainPlugin::menuView:
            dm_viewmenu.push_back(action);
            break;
        case MainPlugin::menuPlugin:
            dm_pluginmenu.push_back(action);
            break;
        default:
            assert(false);
        }
...
QWidget *MainWindow::initLayout(short ty, QWidget *&focuswidget) {
    ...
    switch (ty) {
    case FULL_VIEW: {
        return focuswidget = new MainImageView(this, &dm_leftlist,
                                               &dm_state->dm_imagecache);
    }
    case VCR_VIEW: {
        QVBoxLayout *lay = new QVBoxLayout;
        lay->addWidget(focuswidget = new MainImageView(
                           this, &dm_leftlist, &dm_state->dm_imagecache),
                       1);
    ...
    case STRIP_VIEW: {
        ...
        addDockWidget(
            ...
            dockifyWidget(new ThumbsView(this, &dm_leftlist,
                                         &dm_state->dm_thumbcache, false)));
        break;
    }
    case CATALOG_VIEW: {
        addDockWidget(Qt::LeftDockWidgetArea,
                      dockifyWidget(new ThumbsView(
                          this, &dm_leftlist, &dm_state->dm_thumbcache, true)));

        QVBoxLayout *vlay = new QVBoxLayout;
        vlay->addWidget(focuswidget = new MainImageView(
                            this, &dm_leftlist, &dm_state->dm_imagecache),
                        1);
        ...
        addDockWidget(Qt::RightDockWidgetArea,
                      dockifyWidget(new TagEditorView(this, &dm_leftlist)));
    ...
    case LISTIMAGE_VIEW: {
        ...
        addDockWidget(Qt::LeftDockWidgetArea,
                      dockifyWidget(new DirView(&dm_leftlist)));
        addDockWidget(Qt::LeftDockWidgetArea,
                      first = dockifyWidget(new ListView(this, &dm_leftlist)));
        addDockWidget(Qt::LeftDockWidgetArea,
                      second = dockifyWidget(new ThumbsView(
                          this, &dm_leftlist, &dm_state->dm_thumbcache, true)));
...

        {
            ...
            addDockWidget(Qt::RightDockWidgetArea,
                          second = dockifyWidget(query_widget));
        }
    ...
    case THUMBS_VIEW:
        // old basic view:
        // return new ThumbsView(this, &dm_leftlist, &dm_state->dm_thumbcache,
        // true, true);
        {
            addDockWidget(Qt::LeftDockWidgetArea,
                          dockifyWidget(new DirView(&dm_leftlist)));
