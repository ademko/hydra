...
class MyCheckBox : public QCheckBox {
  public:
    MyCheckBox(const QString &title) : QCheckBox(title) {}

  protected:
    virtual void nextCheckState(void) {
        setCheckState(checkState() == Qt::Unchecked ? Qt::Checked
                                                    : Qt::Unchecked);
    }
};
...
void TagEditorView::updateEditor(void) {
    ...
    for (std::set<QString>::const_iterator ii = dm_recently_added.begin();
         ii != dm_recently_added.end(); ++ii)
        tag_union[*ii] = 0;

    for (int i = 0; i < fileList()->numFiles(); ++i)
        if (fileList()->selectionModel()->isSelected(fileList()->index(i, 0))) {
            hydra::FileItemRecord::tags_t &tags =
                fileList()->fileAt(i).recordItem().tags;
    ...
    // for (union_type::iterator ii=tag_union.begin(); ii!=tag_union.end();
    // ++ii) qDebug() << ii->first << ii->second;

    // remove all the checkboxes that arent in this list (will this every do
    // anything though?)
    ...
    // qDebug() << " L " << dm_containerLayout->geometry() << " bb=" <<
    // dm_containerLayout->boundingBox(); qDebug() << " W " <<
    // dm_containerWidget->geometry();
    /*if
    (!dm_containerWidget->geometry().contains(dm_containerLayout->boundingBox()))
...
void TagEditorView::onCheckBox(bool state) {
    ...
    for (boxes_t::const_iterator ii = dm_boxes.begin(); ii != dm_boxes.end();
         ++ii)
        if (ii->second == src) {
            if (state)
                setTag(ii->first);
...
void TagEditorView::onDataChanged(void) { updateEditor(); }
...
void TagEditorView::initGui(void) {
    ...
    connect(fileList()->selectionModel(),
