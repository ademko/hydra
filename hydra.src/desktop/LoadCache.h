...
class desktop::LoadCacheEntry
{
  public:
    ...
    typename std::list<
      typename std::map<KEY,LoadCacheEntry<T,KEY> >::iterator
...
{
  public:
    explicit LoadCache(size_t maxhold, const LOADER &loader = LOADER()) : LoadCacheBase<T,KEY>(maxhold), dm_loader(loader) { }
...
class desktop::cache_ptr
{
  public:
    ...
    bool operator == (const cache_ptr<T,KEY> &rhs) const { return dm_ptr == rhs.dm_ptr; }
    bool operator != (const cache_ptr<T,KEY> &rhs) const { return dm_ptr != rhs.dm_ptr; }
    bool operator < (const cache_ptr<T,KEY> &rhs) const { return dm_ptr < rhs.dm_ptr; }
...
template <class T, class KEY>
  desktop::cache_ptr<T,KEY> desktop::LoadCacheBase<T,KEY>::getItem(const KEY &key)
{
  typename std::map<KEY, LoadCacheEntry<T,KEY> >::iterator ii(LoadCacheBase<T,KEY>::dm_keymap.find(key));

  assert(ii != (LoadCacheBase<T,KEY>::dm_keymap.end()) );   // need the parens... gcc bug?
...
template <class T, class LOADER, class KEY>
  desktop::cache_ptr<T,KEY> desktop::LoadCache<T,LOADER,KEY>::getItem(const KEY &key)
{
  typename std::map<KEY, LoadCacheEntry<T,KEY> >::iterator ii(LoadCacheBase<T,KEY>::dm_keymap.find(key));
...
template <class T, class KEY>
  desktop::cache_ptr<T,KEY>::cache_ptr(void)
  : dm_cache(0)
{
...
template <class T, class KEY>
  desktop::cache_ptr<T,KEY>::cache_ptr(LoadCacheBase<T,KEY> *cache, typename std::map<KEY,LoadCacheEntry<T,KEY> >::iterator &ii)
...
template <class T, class KEY>
  desktop::cache_ptr<T,KEY> & desktop::cache_ptr<T,KEY>::operator = (const desktop::cache_ptr<T,KEY> &rhs)
{
  //qDebug() << "cache_ptr(assignment)";
  // check if we are already a copy
  if (dm_cache == rhs.dm_cache && (!dm_cache || dm_iterator == rhs.dm_iterator))
    return *this;
...
template <class T, class KEY>
  void desktop::cache_ptr<T,KEY>::cleanup(void)
{
  if (!dm_cache)
    return;

  //qDebug() << "cache_ptr(cleanup) enter, curhold=" << dm_cache->dm_curhold << " maxhold=" << dm_cache->dm_maxhold;
  ...
  if (dm_iterator->second.ptr.unique()) {
    ...
    //qDebug() << __FUNCTION__ << dm_iterator->second.ptr.get() << "ISDEL = true";
    dm_iterator->second.list_iterator = dm_cache->dm_dellist.insert(dm_cache->dm_dellist.end(), dm_iterator);
    ...
    while (dm_cache->dm_curhold > dm_cache->dm_maxhold) {
      ...
      //qDebug() << __FUNCTION__ << dm_cache->dm_dellist.front()->second.ptr.get() << "... purged!";
      dm_cache->dm_keymap.erase( dm_cache->dm_dellist.front() );
      dm_cache->dm_dellist.pop_front();
      dm_cache->dm_curhold--;
      //qDebug() << "cache_ptr(cleanup) just NUKED one, curhold=" << dm_cache->dm_curhold << " maxhold=" << dm_cache->dm_maxhold;
    }
  }
...
{
  public:
    Data(const QString &d) : dm_data("DATA" + d) { qDebug() << __FUNCTION__ << dm_data; }
    ~Data() { qDebug() << __FUNCTION__ << dm_data; }
  private:
    QString dm_data;
};
class ImageLoader
