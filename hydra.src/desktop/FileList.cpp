...
FileSystemWatcher::FileSystemWatcher(FileList *parent) : dm_parent(parent) {
    dm_queued = false;
    dm_requeue = false;
...
FileList::FileList(FileEntryCache *_cache)
    : dm_cache(_cache), dm_peer(0), dm_base_recurse(false),
      dm_base_shuffle(false), /*dm_selectedfile(0),*/ dm_selectionmodel(this),
      dm_watcher(this) {
...
int FileList::rowCount(const QModelIndex &parent) const { return numFiles(); }

int FileList::columnCount(const QModelIndex &parent) const { return 5; }

QVariant FileList::headerData(int section, Qt::Orientation orientation,
                              int role) const {
...
QVariant FileList::data(const QModelIndex &index, int role) const {
    ...
    case 1: {
        ...
        if (needsBigRead) {
            const_cast<FileList *>(this)->enqueueLoad(entry.fullfilename(),
                                                      index.row());
            return "loading...";
        } else {
            QString ret;
            for (FileItemRecord::tags_t::const_iterator ii =
                     entry.recordItem().tags.begin();
    ...
    case 2:
        return entry.fileLastModified().toString(Qt::SystemLocaleShortDate);
    // case 2: return entry.fileLastModified().toString(Qt::ISODate);
    case 3:
        return QString::number(entry.fileSize() / 1024) + "K";
    case 4:
...
void FileList::sort(int column, Qt::SortOrder order) {
    ...
    switch (column) {
    case 0:
        sortby = FileEntryLessThan::colJustName;
        break;
    case 1:
        sortby = FileEntryLessThan::colRecordTags;
        break;
    case 2:
        sortby = FileEntryLessThan::colFileLastModified;
        break;
    case 3:
        sortby = FileEntryLessThan::colFileSize;
        break;
    default:
        return;
    }
...
void FileList::setPeer(FileList *peer) { dm_peer = peer; }
...
int FileList::findFileIndexByName(const QString &fullfilename) const {
    ...
    for (EntryList::const_iterator ii = dm_filtered_files.begin();
         ii != dm_filtered_files.end(); ++ii, ++index)
...
void FileList::setSelectedFile(int index) {
    // qDebug() << "A";
    if (index < 0 || index >= dm_filtered_files.size() ||
        index == selectedFileIndex())
        return;
    // qDebug() << "B";

    // dm_selectedfile = index;
    dm_selectionmodel.setCurrentIndex(FileList::index(index, 0),
                                      QItemSelectionModel::ClearAndSelect);
}

void FileList::addListener(FileListListener *v) { dm_listeners.push_back(v); }
...
void FileList::enqueueLoad(const QString &fullfilename, int idx) {
    ...
    RunnableEventFunction::enqueueWorker(
        std::bind(std::mem_fn(&FileList::metaLoaderFunc), this, fullfilename,
                  idx),
...
void FileList::metaLoaderFuncCommit(const QString &fullfilename, int idx,
                                    const QString &hash) {
...
bool FileEntryLessThan::operator()(
    const desktop::cache_ptr<desktop::FileEntry> &left,
    ...
    case colRecordTags:
        if (dm_order == Qt::AscendingOrder)
            return tagsLessThan(left->recordItem().tags,
                                right->recordItem().tags);
        else
            return tagsLessThan(right->recordItem().tags,
                                left->recordItem().tags);
...
bool FileEntryLessThan::tagsLessThan(
    const hydra::FileItemRecord::tags_t &left,
...
FileListLoader::FileListLoader(FileList &filelist, FileListListener *source,
    ...
    : dm_filelist(filelist), dm_source(source),
      dm_resetselection(resetSelectionTo0) {
...
FileListLoader::~FileListLoader() {
    ...
    if (dm_changed_list || dm_changed_base || dm_changed_base_query) {
        ...
        if (dm_filelist.hasBaseQuery()) {
            ...
            for (FileList::EntryList::const_iterator ii =
                     dm_filelist.dm_all_files.begin();
                 ii != dm_filelist.dm_all_files.end(); ++ii) {
...
int FileListLoader::appendListDir(const QString &dirname) {
    ...
    while (I.hasNext()) {
        ...
        dm_filelist.dm_all_files.push_back(
            dm_filelist.dm_cache->getEntry(curfull));
...
class ReloadEntry {
  public:
    ...
    bool operator<(const ReloadEntry &rhs) const {
        return *fullfilename < *rhs.fullfilename;
    }
};
...
FileListReloader::~FileListReloader() {
    ...
    while (I.hasNext()) {
        ...
        foundii = std::lower_bound(all_full_names.begin(), all_full_names.end(),
                                   targetentry);

        if (foundii != all_full_names.end() &&
            foundii->fullfilename == curfull) {
    ...
    for (i = sizei - 1; i >= 0; --i) {
        if (!all_full_names[i].foundMaker) {
            ...
            nukeset.insert(
                dm_filelist.dm_all_files[all_full_names[i].originalIndex]);

            dm_filelist.dm_all_files.erase(dm_filelist.dm_all_files.begin() +
  ...
  qDebug() << "clear-all-filtered-files";
    ...
    qDebug() << "K" << k;
        while (k < dm_filelist.dm_filtered_files.size() &&
            dm_filelist.dm_all_files[i].get() !=
  dm_filelist.dm_filtered_files[k].get()) { qDebug() << "filtered-removefile: "
  << dm_filelist.dm_all_files[i]->fullfilename(); did_something = true;
          dm_filelist.dm_filtered_files.erase(dm_filelist.dm_filtered_files.begin()
  + k);
        }//while
        ++k;
      }//while
  qDebug() << "done-half";*/
    ...
    while (k < dm_filelist.dm_filtered_files.size()) {
      ReloadEntry targetentry;
      targetentry.fullfilename =
  &dm_filelist.dm_filtered_files[k]->fullfilename();

      foundii = std::lower_bound(all_full_names.begin(), all_full_names.end(),
  targetentry); bool keep_it = false;

      if (foundii != all_full_names.end() && *foundii->fullfilename ==
  *targetentry.fullfilename) { if ((*foundii).foundMaker) keep_it = true;
      }
  ...
  + k);
    ...
    /*qDebug() << "NUKESET";
    for (std::set<cache_ptr<FileEntry> >::iterator oo= nukeset.begin(); oo !=
    nukeset.end(); ++oo) qDebug() << (*oo)->fullfilename(); qDebug() <<
    "/NUKESET";*/
    ...
    while (k < dm_filelist.dm_filtered_files.size()) {
        ...
        if (keep_it)
            ...
            did_something = true;
            qDebug() << "FileListReloader"
                     << "filtered-removefile: "
                     << dm_filelist.dm_filtered_files[k]->fullfilename();
            dm_filelist.dm_filtered_files.erase(
                dm_filelist.dm_filtered_files.begin() + k);
        }
    }
    ...
    for (iinew = new_files.begin(); iinew != new_files.end(); ++iinew) {
        desktop::cache_ptr<FileEntry> entry =
            dm_filelist.dm_cache->getEntry(*iinew);
        dm_filelist.dm_all_files.push_back(entry);

        // qDebug() << "newfile: " << *iinew;
        if (!dm_filelist.hasBaseQuery() ||
            q->isMatch(entry->recordItem().tags)) {
            qDebug() << "FileListReloader"
                     << "filtered-newfile: " << *iinew;
            did_something = true;
