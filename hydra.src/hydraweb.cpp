...
static void showHelp(QTextStream &out) {
    out << "hydraweb -- command line website maker -- "
           "version " HYDRA_VERSION_STRING " (" __DATE__ " " __TIME__ ")\n"
           ...
           "  flat                just dump all the files into the single "
           "output directory (the default)\n"
           "  subdir              use the source files directory when putting "
           "files in the output directory\n"
           "  pivot PIVOT_TAG     use tag pivot placement, on the given tag "
           "(example blah/ to match blah/foo blah/bar etc)\n"
           "  query Q             use the given query to filter results "
           "(default=all)\n"
           "  webpath PATH        place the files in the given web path "
           "(default=emptystring)\n"
...
static QString prepPrefix(const QString &prefix) {
    ...
    if (QFileInfo(prefix).isDir())
        ...
        ret = makeAbsolute("./"); // just use the current dir then, and ignore
                                  // the path in the prefix
...
static bool PivotPath(const QString &pivot_tag,
                      const FileItemRecord::tags_t &tags, QString &outpath) {
    assert(!pivot_tag.isEmpty());

    // find the proper tag
    for (FileItemRecord::tags_t::const_iterator ii = tags.begin();
         ii != tags.end(); ++ii)
...
static void commandWebExport(QTextStream &out, ArgumentParser &parser) {
    ...
    if (outputdir.isEmpty())
        throw ArgumentParser::ErrorException(
            "The output (-o) directory must be specified");

    if (!QFileInfo(outputdir).isDir())
        throw ArgumentParser::ErrorException(outputdir +
                                             " must be a directory");
    ...
    while (j < maxj) {
        ...
        if (param == "flat") {
            ...
            while (ii.hasNext()) {
                ...
                // std::cerr << "prefix=" << dir_prefix.utf8_str() << " cur=" <<
                // fullcur.utf8_str() << endl;
                switch (curtype) {
                case subdir_type:
                    exporter.addFile(
                        fullcur,
                        AppendPath(webpath,
                                   JustPath(stripPrefix(fullcur, dir_prefix))),
                        item, path.hash);
                    break;
                case pivot_type:
