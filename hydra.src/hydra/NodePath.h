...
class NodePath {
  public:
    class error : public std::exception {
        virtual const char *what() const throw() { return "NodePath::error"; }
    };
  ...
  public:
     ...
     * @author Aleksander Demko
     */
    NodePath(QDomDocument doc) : dm_node(doc.documentElement()) {}
    /**
     * Associate a NodePath based off the given node
     *
     * @author Aleksander Demko
     */
    NodePath(QDomElement node) : dm_node(node) {}
    ...
    // apend a new node with the given name (this always adds new nodes). node
    // names need not be unique
    NodePath append(const QString &subpath);
    // erase the given subnode, if it exists, returns true of something was
    // deleted
    bool erase(const QString &subpath);

    // loads the next sibling. return true if it was loaded, false on failure
    // (on failre, "this" is now undefined a sibling is the next sibling node
