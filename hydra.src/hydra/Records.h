 ...
 *
 * These are stored in the cache database, as this
 * information can always be regenerated by
 * rehashing the disk files.
 *
 * key=full_path_name
 * dbfile=cache1.sqlite table=filepaths
 *
 ...
 *
 * This maps a hash string to a file item id.
 * Since a "file" can have multiple hashes,
 * there can be multiple hydra::FileHashRecord
 * pointing to the same hydra::FileItemRecord
 *
 * key=hash_string
...
class hydra::Tags : public std::set<QString> {
  ...
  public:
    ...
    /**
     * Sets the given tag, while maintaining some consistancy (for example, =
     * key values). This is prefered over direct write access to the std::set
     * class. In the future, direct access may be eliminated (or the whole thing
     * replaced with a map?)
     *
     * If imageChanged is non-null AND this operation changes the image (rotate
     * tags), then *imageChanged will be set to true. It will be untouched
     * otherwise.
     *
     * @param tag the tag to insert/set.
     * @param imageChanged was the image "changed"
     * @return true if the tags where modified (and probably need to be
     * commited)
     * @author Aleksander Demko
     */
     ...
     * Note that if there is no =, then it is not a valid key-value. Therefore
     * basic tags will return false for this test.
     *
     * TODO in the future, switch the hole Tags concept to a std::map, which
     * would be optimized for empty-values (as 99% of the tags wont have any
     * values)... or not
     *
     * @author Aleksander Demko
     */
     ...
     * @author Aleksander Demko
     */
    bool insertKey(const QString &key, const QString &value,
                   bool *imageChanged = 0);
     ...
     * @author Aleksander Demko
     */
    static inline int indexOfKeyValue(const QString &s) {
        return s.indexOf(SEP_CHAR);
    }
};

/**
 * The main file record, keyed by their UUID.
 * This contains all the information, like tags,
...
class hydra::FileItemRecord : public hydra::Record {
  public:
    ...
    QUuid id; // my own id, duplicated from the key, to ease processing
    ...
    tags_t tags; // although you can have 0 tags, tag strings themselve cannot
                 // be empty (0-length)
     ...
     *
     * (move this to the DB layer eventually? - probably not, as this is very
     * specific)
     * @author Aleksander Demko
     */
    QDateTime modtime;
  ...
  public:
    ...
    quint64 modtimeAsInt64(void) const { return dateTimeAsInt64(modtime); }
    void setModtimeFromInt64(quint64 i) { int64ToDateTime(i, modtime); }
};
