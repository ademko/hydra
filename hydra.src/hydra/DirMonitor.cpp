...
void hydra::pruneDirectory(const QString &dirname)
{
  ...
  while (C.next()) {
    ...
    // this will SKIP all the sub-directories, as they will show up in the prefix match
    // (this is a hack until we/maybe we implement a directory-centric filedirpath system DB
    if (curkey.lastIndexOf('/') >= prefix_len)
      continue;
    //qDebug() << "key=  " << curkey << curkey.lastIndexOf("/") << prefix_len;

    if (!QFileInfo(curkey).exists())
      eraselist.push_back(curkey);
  }

  for (QStringList::const_iterator ii=eraselist.begin(); ii != eraselist.end(); ++ii) {
    qDebug() << "PRUNE" << *ii;
...
{
  qDebug() << "addFiles:";
  for (QStringList::const_iterator ii = addFiles.begin(); ii != addFiles.end(); ++ii)
    qDebug() << *ii;
  qDebug() << "removeFiles:";
  for (QStringList::const_iterator ii = removeFiles.begin(); ii != removeFiles.end(); ++ii)
    qDebug() << *ii;
}
...
DirMonitor::DirMonitor(void)
{
  dm_queued = false;
  dm_requeue = false;
  connect(&dm_watcher, SIGNAL(directoryChanged(const QString &)), this, SLOT(onDirChanged(const QString &)));
}
...
void DirMonitor::reloadDirs(void)
{
  ...
  for (marklists_t::iterator ii = dm_marklists.begin(); ii != dm_marklists.end(); ++ii)
...
void DirMonitor::reloadDir(MarkList &marklist, DirChangeEvent &outevent)
{
  ...
  for (MarkList::namelist_t::iterator ii = marklist.foundNames.begin(); ii != marklist.foundNames.end(); ++ii)
    ii->second = false;
  ...
  for (QStringList::const_iterator ii = outevent.removeFiles.begin(); ii != outevent.removeFiles.end(); ++ii)
    marklist.foundNames.erase(*ii);
  for (QStringList::const_iterator ii = outevent.addFiles.begin(); ii != outevent.addFiles.end(); ++ii)
...
void DirDBUpdater::dirChanged(const QString &path, const DirChangeEvent &event)
{
  //qDebug() << __FUNCTION__;

  for (QStringList::const_iterator ii = event.removeFiles.begin(); ii != event.removeFiles.end(); ++ii)
    Engine::instance()->filePathDB().erase(*ii);
  for (QStringList::const_iterator ii = event.addFiles.begin(); ii != event.addFiles.end(); ++ii)
