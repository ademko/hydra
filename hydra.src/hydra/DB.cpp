...
#define sqlite3_prepare_vX(a,b,c,d,e) sqlite3_prepare_v2((a), (b), (c), (d), (e))
#else
#define sqlite3_prepare_vX(a,b,c,d,e) sqlite3_prepare((a), (b), (c), (d), (e))
#endif
...
{
  if (handle)
    sqlite3_close(handle);    // this should be called even if there was an error on open
}
  ...
  : dm_connection(new Connection), dm_table(tablescope)
{
  ...
  //dm_get_hotbuf.reserve(8*1024);    // dm_get_hotbuf will always use fromRawData
...
bool DB::contains(const char *key)
{
  ...
  sqlite3_bind_text(dm_has_statement.handle, 1, key, static_cast<int>(strlen(key)), SQLITE_STATIC);
...
bool DB::insert(const char *key, const std::string &value)
{
  ...
  sqlite3_bind_text(dm_put_statement.handle, 1, key, static_cast<int>(strlen(key)), SQLITE_STATIC);
  sqlite3_bind_blob(dm_put_statement.handle, 2, &value[0], static_cast<int>(value.size()), SQLITE_STATIC);
...
bool DB::get(const char *key, std::string &value)
{
  ...
  sqlite3_bind_text(dm_get_statement.handle, 1, key, static_cast<int>(strlen(key)), SQLITE_STATIC);
  ...
  if (bytes == 0)
    ...
    value.assign(static_cast<const char*>(sqlite3_column_blob(dm_get_statement.handle, 0)), bytes);
...
bool DB::insert(const char *key, const hydra::Record &value)
{
  ...
  QDataStream out_stream(&dm_put_hotbuf, QIODevice::Truncate | QIODevice::WriteOnly | QIODevice::Unbuffered);
  ...
  sqlite3_bind_text(dm_put_statement.handle, 1, key, static_cast<int>(strlen(key)), SQLITE_STATIC);
  sqlite3_bind_blob(dm_put_statement.handle, 2,
      dm_put_hotbuf.constData(), dm_put_hotbuf.size(), SQLITE_STATIC);
...
bool DB::get(const char *key, hydra::Record &value)
{
  ...
  try {
    QDataStream in_streamer(&dm_get_hotbuf, QIODevice::ReadOnly | QIODevice::Unbuffered);
  ...
  : dm_isvalid(false), dm_connection(db.dm_connection)
{
  ...
  sqlite3_bind_text(dm_select_statement.handle, 1, dm_prefix_string.constData(),
      dm_prefix_string.size(), SQLITE_STATIC);
...
{
  assert(dm_isvalid);
  return reinterpret_cast<const char *>(sqlite3_column_text(dm_select_statement.handle, 0));
}
...
void Cursor::get(std::string &value)
{
  ...
  if (bytes == 0)
    ...
    value.assign(static_cast<const char*>(sqlite3_column_blob(dm_select_statement.handle, 1)), bytes);
}
...
void Cursor::get(QString &value)
{
  ...
  if (bytes == 0)
    ...
    value = QString::fromUtf8(static_cast<const char*>(sqlite3_column_blob(dm_select_statement.handle, 1)), bytes);
}
