...
struct loadeditem_t {
    ...
    QUuid existingid; // if !isNull, this is the real ID of the file already in
                      // the system
};
 ...
 * @author Aleksander Demko
 */
static bool supersetMergeItem(const FileItemRecord &src,
                              FileItemRecord &existingdest) {
    bool changed = false;
    ...
    for (FileItemRecord::tags_t::const_iterator tt = src.tags.begin();
         tt != src.tags.end(); ++tt) {
        assert(!tt->isEmpty());
    ...
    if (!scoretag.isEmpty())
        for (FileItemRecord::tags_t::iterator tt = existingdest.tags.begin();
             tt != existingdest.tags.end(); ++tt) {
            if ((*tt)[0] == '$') {
                // found the score tag
    ...
    // if we still have a scoretag, just insert it as the dest didnt have one at
    // all
    if (!scoretag.isEmpty()) {
 ...
 * @author Aleksander Demko
 */
static bool modtimeMergeItem(const FileItemRecord &src,
                             FileItemRecord &existingdest) {
    QDateTime nulltime;
...
MyHandler::MyHandler(bool smartmerge) : dm_smartmerge(smartmerge) {}
...
bool MyHandler::startElement(const QString &namespaceURI,
                             const QString &localName, const QString &qName,
                             const QXmlAttributes &atts) {
    // std::cerr << "localName=" << localName.toUtf8().constData() << " qName="
...
int MyHandler::commit(void) {
    ...
    while (hh != endhh) {
        if (eng->fileHashDB().get(hh->first, hash)) {
            ...
            if (tt != dm_itemmap.end())
                tt->second.existingid =
                    hash.id; // make the loaded item be merged with the already
                             // existing item
    ...
    for (itemmap_t::iterator ii = dm_itemmap.begin(); ii != dm_itemmap.end();
         ++ii) {
        bool domerge;
