...
int Engine::addFile(const QString &fullfilename, const QString *precalchash) {
    ...
    if (dm_filepathdb->get(fullfilename, path) && !precalchash) {
        ...
        if (disk_time == path.file_time && disk_size == path.file_size) {
            // PATH is synced with disk, now just make sure a HASH and ITEM
            // exist
    ...
    if (old_path_itemid.isNull())
        // TODO in the future, replace this with libuui under Linux, as this
        // needs to be STRONG
...
int Engine::getFileItemByHash(const QString &hashkey,
                              hydra::FileItemRecord *item,
                              hydra::FileHashRecord *hash) {
    FileItemRecord localitem;
    FileHashRecord localhash;
...
bool Engine::statFile(const QString &fullfilename, double &modtime,
                      qint64 &filesize) {
