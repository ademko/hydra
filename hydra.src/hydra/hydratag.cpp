...
{
  out <<
    "hydratag -- command line tool -- version " HYDRA_VERSION_STRING " (" __DATE__ " " __TIME__ ")\n"
    ...
    "               -u                  just update/freshen files already in the db\n"
    "               -t tag --tag=tag    also add the given tag for the files\n"
    "  list       lists the database information for the given files\n"
    "               -q --query=exp      filter by the given tag query string\n"
    "               -a                  show all files, even those not in db\n"
    "               -f                  format: list only files\n"
    "               -x                  format: xml output format\n"
    "               -0                  format: use null strings, suitable for xargs -0\n"
    ...
    "                  A test involves finding all image files and hashing them,\n"
    "                  then verifying that that hash is in the DB.\n"
    "                  This function can be used to thoroughly check backups.\n"
    "  dupe       finds and notes all the dupes in the give directories/files\n"
    "               -a show all files, not just those that are dupes\n"
    "               -h show only those files that have multiple hashes\n"
    "                  (this will not show those files that just have path dupes)\n"
    "               -D DELETEs hash duplicate files. This will only keep one file\n"
    "                  of every unique hash\n"
    "  touch      rehashs all the given files. this is identical to a \"forget\"\n"
    "             followed by an \"add\" (the add will just rehash the files)\n"
    "  forget     forgets the path associations with the given directories/files\n"
    "             NOTE! forget is usually a developer function and usually should\n"
    "             not be needed by regular users.\n"
    "               -h forgets the hash association too (NOTE! this will untie\n"
    "                  the files with their current item records.)\n"
    "                  Upon readdition, the files will have new, blank item records.\n"
    "\n"
    "Typical tag styles:\n"
    ...
    "  TAG*       true if contains tag that starts with TAG (prefix match)\n"
...
static void commandAdd(QTextStream &out, ArgumentParser &parser)
{
  ...
  if (fileList.isEmpty())
    throw ArgumentParser::ErrorException("Specify one or more files or directories");

  // perform the add
  for (QStringList::const_iterator ii=fileList.begin(); ii != fileList.end(); ++ii) {
    FileIterator I(*ii);
    ...
    while (I.hasNext()) {
      ...
      if (addcode != Engine::Add_Error) {
        ...
        if (code == Engine::Load_OK) {
          ...
          for (std::set<QString>::const_iterator ii=add_tags.begin(); ii != add_tags.end(); ++ii)
...
static void commandList(QTextStream &out, ArgumentParser &parser)
{
  ...
  for (QStringList::const_iterator ii=fileList.begin(); ii != fileList.end(); ++ii) {
    ...
    while (I.hasNext()) {
      ...
      int code = Engine::instance()->getFileItem(fullcur, &item, &hash, &path);

      if (list_all && code == Engine::Load_ErrorFatal)
        continue;
      if (!list_all && (code == Engine::Load_ErrorFatal || code == Engine::Load_ErrorNotFound))
        continue;
      ...
      if (format_xml) {
        if (code != Engine::Load_OK)
          continue;   // xml format cannot do files that are not in the db

        out << "<path filename=\"" << escapeForXML(fullcur) <<
          "\" hash=\"" << path.hash << "\" />" << endl;
        out << "<hash hash=\"" << path.hash << "\" id=\"" << hash.id << "\" />" << endl;
        out << "<item id=\"" << hash.id
          << "\" title=\"" << escapeForXML(item.title)
          << "\" desc=\"" << escapeForXML(item.desc)
          << "\" filetype=\"" << item.filetype
          << "\" modtime=\"" << item.modtimeAsInt64()
          << "\" modtime_iso_local=\"" << item.modtime.toString(Qt::ISODate)
          << "\" modtime_iso_utc=\"" << item.modtime.toUTC().toString(Qt::ISODate)
          << "\" >";
        for (FileItemRecord::tags_t::iterator ii=item.tags.begin(); ii != item.tags.end(); ++ii)
      ...
      out << " filetype=" << item.filetype
          << " rotatecode=" << detectMultiRotate(fullcur, item.tags) // rotate code
          << " {";
      for (FileItemRecord::tags_t::iterator ii=item.tags.begin(); ii != item.tags.end(); ++ii)
        out << ' ' << *ii;
      out << " }" << endl;
...
static void commandTag(QTextStream &out, ArgumentParser &parser)
{
  ...
  for (QStringList::const_iterator ii=fileList.begin(); ii != fileList.end(); ++ii) {
    ...
    while (I.hasNext()) {
      ...
      for (std::set<QString>::const_iterator ii=rm_tags.begin(); ii != rm_tags.end(); ++ii)
...
static void commandExport(QTextStream &out, ArgumentParser &parser)
{
  ...
  if (!file.open(QIODevice::WriteOnly))
    throw ArgumentParser::ErrorException("can't open file for writing: " + filename);
  ...
  }
  {
    ...
    while (cur.next()) {
      ...
      outfile << "<hash hash=\"" << cur.getKey() << "\" id=\"" << hash.id << "\" />" << endl;
    }//while
  }
  {
    FileItemRecord item;
...
static void commandImport(QTextStream &out, ArgumentParser &parser)
{
  ...
  if (count == -1)
    throw ArgumentParser::ErrorException("failed to import data from: " + filename);
...
static void commandTest(QTextStream &out, ArgumentParser &parser)
{
  ...
  for (QStringList::const_iterator ii=fileList.begin(); ii != fileList.end(); ++ii) {
    ...
    while (I.hasNext()) {
      ...
      if (isNormalFile(justname) && isImageFile(justname)) {
        ...
        if (curok)
          if (Engine::instance()->getFileItemByHash(filehash, 0, 0) != Engine::Load_OK)
            curok = false;
  ...
  if (failed.empty())
    ...
    for (failed_t::const_iterator ii=failed.begin(); ii != failed.end(); ++ii)
      out << *ii << endl;
...
static void commandDupe(QTextStream &out, ArgumentParser &parser)
{
  ...
  for (idhashmap_t::iterator ii=idhash.begin(); ii != idhash.end(); ++ii) {
    ...
    if (show_all)
      ...
      for (stringset_t::iterator hh=ii->second.begin(); hh != ii->second.end(); ++hh)
    ...
    if (printed_item) {
      ...
      for (stringset_t::iterator hh=ii->second.begin(); hh != ii->second.end(); ++hh) {
        ...
        for (stringset_t::iterator pp = S.begin(); pp != S.end(); ++pp) {
          //out << "    " << *pp << '\n';
          if (nuke_this) {
            out << " D  " << *hh << "    " << *pp << '\n';
            delFile(*pp);
...
static void commandTouch(QTextStream &out, ArgumentParser &parser)
{
  ...
  for (QStringList::const_iterator ii=fileList.begin(); ii != fileList.end(); ++ii) {
    ...
    while (I.hasNext()) {
      ...
      did_something = Engine::instance()->erasePath(fullcur) || did_something;
...
static void commandForget(QTextStream &out, ArgumentParser &parser)
{
  ...
  for (QStringList::const_iterator ii=fileList.begin(); ii != fileList.end(); ++ii) {
    ...
    while (I.hasNext()) {
      ...
      did_something = Engine::instance()->erasePath(fullcur) || did_something;
      if (de_hash)
        did_something = Engine::instance()->eraseHash(path.hash) || did_something;
