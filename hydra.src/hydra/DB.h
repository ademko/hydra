...
class hydra::DB {
  public:
     ...
     * file and prepend the given prefix/namespace to the tables.
     *
     * This constructor allows you to reuse anoter DB instance's sqlite
     * connection.
     *
     * @param linked_db the DB from whome this DB will share a single sqlite
     * connection.
     * @param tablescope the prefix/table name to use. Cannot be empty.
     * @author Aleksander Demko
     */
    ...
    bool insert(const QString &key, const std::string &value) {
        return insert(key.toUtf8().constData(), value);
    }
    /// saves to the db, returns true on success, will replace if exists
    ...
    bool get(const QString &key, std::string &value) {
        return get(key.toUtf8().constData(), value);
    }
    /// returns false on not-found or failed to load
  ...
  private:
    ...
    Statement dm_put_statement, dm_has_statement, dm_get_statement,
        dm_erase_statement;
...
class hydra::Cursor {
  public:
     ...
     * Loads the current value. The current row must be valid.
     *
     * Returns false if the record failed to deserialize (ie. it threw an
     * exception)
     *
     * @author Aleksander Demko
     */
