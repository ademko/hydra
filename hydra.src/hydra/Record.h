...
class hydra::Record
{
  ...
  private:
    friend QDataStream & ::operator << (QDataStream & stream, const hydra::Record &rec);
    friend QDataStream & ::operator >> (QDataStream & stream, hydra::Record &rec);
};

// now declare their bodies as theyre inline
inline QDataStream & operator << (QDataStream & stream, const hydra::Record &rec) { rec.save(stream); return stream; }
inline QDataStream & operator >> (QDataStream & stream, hydra::Record &rec) { rec.load(stream); return stream; }
