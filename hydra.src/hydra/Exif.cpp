...
short hydra::detectExifRotate(const QString &filename)
{
  ...
  dm_exif_rotate = 0;
  {
    ...
    if (edat && (een = exif_content_get_entry(edat->ifd[EXIF_IFD_0], EXIF_TAG_ORIENTATION))) {
      exif_entry_ref(een);
...
class CheckedFile : public QFile
{
  ...
  public:
    ...
    void checkedOpen(OpenMode mode) { if (!QFile::open(mode)) throw Error("Can't open file"); }
